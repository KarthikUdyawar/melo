...
describe("previewSong", () => {
  it("unwraps envelope body on 200", async () => {
    ...
    const result = await api.previewSong(
  ...
  it("throws ApiError instance, not a plain object", async () => {
    server.use(
      http.post("/api/songs/preview", () =>
        HttpResponse.json(
          { status_code: 502, message: "yt-dlp fetch failed", body: {} },
          { status: 502 },
        ),
      ),
    );
...
describe("reorderSongInPlaylist", () => {
  ...
  it("throws 422 for out-of-range position", async () => {
    server.use(
      http.patch("/api/playlists/:playlistId/songs/:songId", () =>
        HttpResponse.json(
          { status_code: 422, message: "position out of range", body: {} },
          { status: 422 },
        ),
      ),
    );
    ...
    await expect(
      api.reorderSongInPlaylist("p1", "s1", 99),
...
describe("favorites — listFavorites", () => {
  it("GETs and unwraps paginated body", async () => {
    server.use(
    ...
    await expect(api.listFavorites()).resolves.toEqual({
      records: [],
      count: 0,
      bookmark: null,
    });
...
describe("playlists — create/list/get/delete", () => {
  it("round-trips through the envelope", async () => {
    server.use(
      ...
      http.get("/api/playlists/:id", () =>
        HttpResponse.json({
          status_code: 200,
          message: "ok",
          body: { id: "p1", name: "Mix", created_at: "t", songs: [] },
        }),
      ),
      http.delete(
        "/api/playlists/:id",
    ...
    await expect(api.createPlaylist("Mix")).resolves.toMatchObject({
      id: "p1",
    });
    await expect(api.listPlaylists()).resolves.toMatchObject({ count: 0 });
    await expect(api.getPlaylist("p1")).resolves.toMatchObject({ id: "p1" });
    await expect(api.deletePlaylist("p1")).resolves.toBeUndefined();
  });
});

describe("playlists — add/remove song", () => {
  it("addSongToPlaylist POSTs, removeSongFromPlaylist DELETEs", async () => {
    server.use(
    ...
    await expect(api.addSongToPlaylist("p1", "s1")).resolves.toEqual({
      id: "s1",
    });
    await expect(
      api.removeSongFromPlaylist("p1", "s1"),
    ).resolves.toBeUndefined();
  });
});

describe("apiFetch error fallback", () => {
  it("non-JSON error response falls back to statusText message", async () => {
    server.use(
      http.get(
        "/api/songs/:id",
        () =>
    ...
    await expect(api.getSong("x")).rejects.toBeInstanceOf(ApiError);
  });
});
