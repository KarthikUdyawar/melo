...
def mock_processor():
    with (
        patch("app.api.songs.trim_audio") as m_trim,  # ← not processor module
        patch("app.api.songs.apply_speed") as m_speed,  # ← not processor module
...
class TestListSongs:
    ...
    def test_records_contain_expected_fields(
        ...
        for field in ("id", "status", "youtube_id", "created_at", "speed"):
...
class TestStreamProcessing:
    def test_stream_with_trim_and_speed(
        ...
        # Use 'with patch' here to ensure the local import inside the endpoint
        # picks up our mock from the processor module
...
class TestSongEdgeCases:
    def test_stream_minio_fetch_error(self, client: TestClient, db_session: Session):
        ...
        # Use .get() to avoid KeyError and see what the body actually is if it fails
        body = resp.json()
