...
- `PATCH /playlists/{id}/songs/{song_id}` — reorder a song to a given 0-indexed position, shifts intermediate songs' positions (not a swap); `200`/`404`/`422`, no `409` (sentinel-position shift runs inside one transaction)
